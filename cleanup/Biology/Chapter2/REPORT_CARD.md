# REPORT_CARD — Chapter 2: Chemistry of Life
...
|------|------|------------|-------------------|-----------------|------|----------------------------------|
| Chapter 2 notes.pptx | Slides (35 slides, all 5 sections — broken into daily use) | section 2.3 → **HS-LS1-6** (carbon → large carbon-based molecules); 2.1 foundational chem; 2.2 foundational substrate for HS-LS1-3 (water → homeostasis); 2.4 foundational substrate for HS-LS1-5/1-7 (energy in reactions); 2.5 foundational, ties to HS-LS1-3 (enzymes & homeostasis); CC: Energy & Matter (2.3–2.4), Structure & Function (2.5) | content only throughout | Content source for the bolt-on — the 2.3 portion (carbs/lipids/proteins/nucleic acids) presents the four macromolecules as definitions, but HS-LS1-6 wants students to *construct and revise an explanation*. The bolt-on is delivered as a **CFU 2.3-A/2.3-B** Forms item, not a deck edit | Canonical deck — per-section breakout decks exist as working copies (see Cleanup flags below) | **Two lab hooks.** 2.3 macromolecules → **food-testing lab**: Biuret (protein) and iodine (starch) tests fully equipped — Carolina Biuret reagent, iodine, soluble starch, glucose in stock (§5); Benedict's (sugars) reagent not confirmed — check §5/§14. 2.5 enzymes → **enzyme-activity lab**: Carolina lipase in stock (§5). Glassware: test tubes, racks, droppers (§3–4) on hand |
...
| Biology CFU 2.3 - A.docx | CFU | **HS-LS1-6** — carbohydrates, polymer structure | content only (fill-in-blank recall) | CFU revision (Forms) — this is where HS-LS1-6 lives; change "what are the three structures" → "explain how carbon's bonding lets it build these large molecules" as a **[short-answer]** item, keep the recall items **[auto-grade]** | Higher-value revision target | None built-in; pairs with the 2.3 macromolecule food-testing lab (reagents §5) |
| Biology CFU 2.3 - B.docx | CFU | **HS-LS1-6** — lipids, proteins, nucleic acids | content only (fill-in-blank recall) | CFU revision (Forms) — same as 2.3-A; add one **[short-answer]** macromolecule-explanation item | Pairs with 2.3-A | None built-in; pairs with the 2.3 food-testing lab — Biuret test directly checks the proteins covered here (§5) |
...
**Chapter verdict:** 9 of 11 in-scope files leave-be — Chapter 2 is largely foundational chemistry substrate (atoms, water, reactions, enzymes), which is doing its job as groundwork for the HS-LS1 process standards. The single real PE hit is **section 2.3, carbon-based molecules → HS-LS1-6**, and that's the chapter's CER bolt-on, delivered as CFU Forms revisions: the content is in the main deck's 2.3 slides, and the revision lands in **CFU 2.3-A and 2.3-B** — one **[short-answer]** explanation item each, added to the otherwise auto-graded section Form.
...
**Resolved 2026-06-11** — exact content-identical copies removed (each re-verified identical before deletion; kept the cleaner filename in each pair/group):

- Section 2.4 deck: removed `Notes -2.4 - Chemical Reactions.pptx` and `Section 2.4 Notes(1).pptx` — kept `Section 2.4 Notes.pptx`
- CFU 2.4: removed `Biology CFU 2.4(1).docx` — kept `Biology CFU 2.4.docx`
- Section 2.3-A deck: removed `Notes -2.3 - A - carbon based molecules.pptx` — kept `Chapter 2.3 - A Notes.pptx`
- Section 2.3-B deck: removed `Notes -2.3 - B - carbon based molecules.pptx` — kept `Chapter 2.3 - B Notes.pptx`
- `MicroscopeLabWorksheet-1.pdf` — moved to Chapter 1 (belongs with the Microscope Lesson pre-lab deck)

**Still open (your call — not removed, since none are exact duplicates):**

- **`2CFU 2.1(1).docx`** — near-duplicate of `2CFU 2.1.docx` but lacks the point-value annotations (superseded, not byte-identical). Remove if you want, but I left it since you said exact duplicates only.
- **`Chapter 2.3 - AI.pptx`** — an AI-generated rebuild of your vetted 2.3 conveyance (same content, restructured). The "rebuild the core" pattern set aside in project decisions; flagging rather than treating it as core material.
- **`Notes -2.3 - carbon based molecules.pptx`** (12 slides) — a combined A+B version of the 2.3 deck; redundant with the per-section pieces if the 35-slide main deck is canonical.
- **Doc-format notes** (`Chapter 2 notes.docx`, `Chapter 2.1 and 2.2 notes.docx`, `Biology Lecture Notes - Chapter 2.docx`, `lecture notes.docx`) — text versions parallel to the slide deck; out of audit scope, noted for cleanup awareness.
