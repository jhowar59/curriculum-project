...
**Coverage gap — the LS2 ecology strand (confirmed a Biology responsibility).** Checking the course badges in the original ND 2019 PDF (pp. 180–187) settled this: **all eight HS-LS2 standards carry the Biology badge** — ecology *is* part of the Biology course. *(Correction: an earlier draft of this section said six LS2 standards were "shared with Environmental Science" and that LS2-3/LS2-5 were "Biology-only." That was an icon misread — the second badge on those pages is the **ND Connections** mark (a wheat stalk), not an Environmental Science badge. The LS standards carry no course-label text and never reference Environmental Science, so all that's confirmed is the Biology assignment; whether Env Sci also covers any LS2 standard is unverified. It doesn't change the Biology obligation either way.)* Status in the audited chapters:
- **HS-LS2-3** (aerobic/anaerobic cycling) — **covered** in Ch4 (cellular energy). ✓
- **HS-LS2-1, 2, 4, 5, 6, 7, 8** and **HS-LS4-6** — **not covered** in the audited Ch1–11 arc.

**This is a genuine Biology coverage gap — with one likely-benign explanation.** The audited chapters (Ch1–8, 10, 11) are the molecules→cells→genetics→evolution arc; in a typical HMH sequence ecology lives in *later* chapters. So either (a) ecology chapters exist beyond Ch11 that weren't in this cleanup folder and simply weren't audited, or (b) the course doesn't reach ecology (consistent with the end-of-year pacing squeeze that also dropped Ch9). **Action: confirm where the ecology unit lives and whether it's taught.** If it exists, point the audit at it; if it's not being reached, LS2 is the real standards gap for the course, not HS-LS4-3. *(Resolved later: the course stops near Ch11–12 of 23, so ecology is never reached — see the build items.)*
