# Physical Science — Supplies to Check / Order

Running list of items flagged during the Chapter audit. Nothing here is urgent unless a lab is being scheduled. Items marked **confirm** just need a storage check; items marked **order** are genuinely not in inventory.

---

## Ch2 — Classifying Matter

| Item | Lab | Status | Notes |
|------|-----|--------|-------|
| Metal samples — solid pieces of Al, Fe, Cu, Zn (+ Ni/Sn if available) | Physical Properties of Metals lab | **check storage** | Teacher has various metals on hand in unknown forms. **Powders won't work** (can't test conductivity, hardness, or density cleanly). Sheets and solid chunks are fine. Note: INVENTORY shows iron as *powder* — use a steel nail as the iron sample. **Nails confirmed on hand (plenty).** Al, Cu, Zn solid pieces probably on hand; check. A 3-metal set (Al + Fe/steel + Cu) fully demonstrates the concept if some are missing. |
| Hot plate | Physical Properties of Metals lab (conductivity test step) | ✅ confirmed | In INVENTORY §2 ("photo missed") |

---

## Ch3 — States of Matter

| Item | Lab | Status | Notes |
|------|-----|--------|-------|
| Rock salt | Boiling/Freezing publisher lab (freezing half) | **confirm** | Only needed for the full 2-period version (coffee-can ice bath); custom 1-period boiling lab doesn't need it |
| Hot plate | Boiling/Freezing lab | ✅ confirmed | In INVENTORY §2 |

---

## Ch4 — Atoms

| Item | Lab | Status | Notes |
|------|-----|--------|-------|
| Plastic foam balls, ~2–3 cm, blue (×6) + white (×4) | Building Isotopes lab | **order if not on hand** | Used to build nuclear models; toothpicks are already in Gen supplies; foam balls are cheap craft-store item |

---

## Ch5 — The Periodic Table

| Item | Lab | Status | Notes |
|------|-----|--------|-------|
| Carbon/graphite electrode rods | Conductivity Lab ("Which Solutions Make Ions?") | **confirm quantity** | ~8 on hand per teacher — check if that's enough for class size; pencil-lead cores work as a substitute if short; graphite rods are inexpensive |

---

*Updated as new chapters are audited. Last updated: 2026-06-11 (Ch1–Ch5 complete).*
